...
func (r *DesignateReconciler) reconcileNormal(ctx context.Context, instance *designatev1beta1.Designate, helper *helper.Helper) (ctrl.Result, error) {
	...
	bindReady := instance.Status.DesignateBackendbind9ReadyCount == *instance.Spec.DesignateBackendbind9.Replicas
	mdnsReady := instance.Status.DesignateMdnsReadyCount == *instance.Spec.DesignateMdns.Replicas
	servicesReady := instance.Status.DesignateCentralReadyCount > 0 && bindReady && mdnsReady
	if len(nsRecordsConfigMap.Data) > 0 && servicesReady {
